...
pennant_annotations = read_csv('/Users/yannryan/Downloads/5x7kygxisii40w (7).csv')
...
pennant_annotations = pennant_annotations %>% mutate(context = text_vec)
...
l = leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron) %>% setView(50, 30, zoom = 2)%>% 
  addPolygons(data = all_countries_sf, popup = ~context, fillColor = 'lightblue', color = 'black', weight = 2, group = 'Countries')%>% 
  addPolygons(data = all_provinces,  popup = ~context, fillColor = 'forestgreen', color = 'black', weight = 2, group = 'Provinces')  %>% 
  addCircleMarkers(data = pennant_full_sf, label = ~VOCAB_LABEL,  popup =~context ,
                   radius = 5, weight = 1, opacity = 1, fillOpacity = .9, color = 'black', fillColor = '#d9d9d9', group = 'Places (cities)') %>%
  # Layers control
  addLayersControl(
    overlayGroups = c('Countries', 'Provinces', 'Places (cities)'),
...
xavier = xavier %>% 
  mutate(extra_coords = str_extract(COMMENTS, "([-+]?\\d{1,2}[.]\\d+),\\s*([-+]?\\d{1,3}[.]\\d+)")) %>% 
...
xavier_leaflet = leaflet() %>%
  ...
  addCircleMarkers(data = xavier_sf,  popup =~context ,
...
polo_leaflet = leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron)  %>% 
  addCircleMarkers(data = polo_sf,  popup =~context ,
                   radius = 5, weight = 1, opacity = 1, fillOpacity = .9, color = 'black', fillColor = '#d9d9d9')
...
ricci_leaflet = leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron)  %>% 
  addCircleMarkers(data = ricci_sf,  popup =~context ,
                   radius = 5, weight = 1, opacity = 1, fillOpacity = .9, color = 'black', fillColor = '#d9d9d9')
...
andrade_leaflet = leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron)  %>% 
  addCircleMarkers(data = andrade_sf,  popup =~context ,
                   radius = 5, weight = 1, opacity = 1, fillOpacity = .9, color = 'black', fillColor = '#d9d9d9')
...
nieuhof_leaflet = leaflet()  %>% setView(120, 30, zoom = 5) %>%
  ...
  addCircleMarkers(data = nieuhof_sf,  popup =~context ,
                   radius = 5, weight = 1, opacity = 1, fillOpacity = .9, color = 'black', fillColor = '#d9d9d9')
...
pennant_coastal_leaflet = leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron)  %>% 
  addPolygons(data = pennant_provinces, popup = ~QUOTE_TRANSCRIPTION, fillColor = 'forestgreen', color = 'black', weight = 2) %>% 
  addCircleMarkers(data = pennant_coastal_sf,  popup =~context ,
                   radius = 5, weight = 1, opacity = 1, fillOpacity = .9, color = 'black', fillColor = '#d9d9d9') %>% 
